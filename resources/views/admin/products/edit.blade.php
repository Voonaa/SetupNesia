<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-2 text-sm text-slate-400">
            ...
            <span>&middot;</span>
            <span class="text-slate-200">Edit Product</span>
        </div>
            ...
            <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1.5 block w-full" :value="old('name', $product->name)" required autofocus placeholder="e.g. Keychron Q1 Pro Wireless Keyboard" />
                    ...
                    <div>
                        ...
                        <x-text-input id="price" name="price" type="number" step="0.01" class="mt-1.5 block w-full" :value="old('price', $product->price)" required placeholder="e.g. 2899000" />
                    ...
                    <div>
                        ...
                        <x-text-input id="stock" name="stock" type="number" class="mt-1.5 block w-full" :value="old('stock', $product->stock)" required placeholder="e.g. 15" />
                    ...
                    <div>
                        ...
                        <x-text-input id="weight" name="weight" type="number" class="mt-1.5 block w-full" :value="old('weight', $product->weight)" required placeholder="e.g. 1800" />
                    ...
                    @if($product->primaryImage)
                        <div class="md:col-span-2">
                            <x-input-label :value="__('Current Primary Image')" />
                            <div class="mt-2 w-48 h-32 rounded-xl bg-slate-800 overflow-hidden border border-slate-700">
                                <img src="{{ $product->primaryImage->image_path }}" alt="Preview" class="w-full h-full object-cover">
                            </div>
                        </div>
                    @endif
                    ...
                    <!-- Image Upload -->
                    <div class="md:col-span-2">
                        <x-input-label for="image" :value="__('Replace Primary Image (Optional)')" />
                        <div class="mt-1.5 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-700 border-dashed rounded-xl bg-slate-900/50 hover:bg-slate-900 transition duration-150">
                            <div class="space-y-1 text-center">
                                ...
                                <p class="text-xs text-slate-500">PNG, JPG, JPEG up to 2MB (Leave empty to keep current image)</p>
                            </div>
                        </div>
                    ...
                    <div class="md:col-span-2 flex items-center">
                        ...
                        <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded bg-slate-900 border-slate-700 text-purple-600 shadow-sm focus:ring-purple-500 focus:ring-offset-slate-900" {{ old('is_active', $product->is_active) ? 'checked' : '' }}>
